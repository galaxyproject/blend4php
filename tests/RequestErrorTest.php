...
class RequestErrorTest extends PHPUnit_Framework_TestCase {
  ...
  // Construct Dummy object
    ...
    $this->assertEquals('Galaxy', $error['type']);
    $this->assertEquals('Unknown CURL response: not json compatible.', $error['message']);
    ...
    $error = $handler->getError();
    $this->assertEquals('Galaxy', $error['type']);
    $this->assertEquals('API authentication required for this request', $error['message']);
    ...
    $error = $handler->getError();
    $this->assertEquals('Galaxy', $error['type']);
    $this->assertEquals('A galaxy error was detected, though its contents are unkown', $error['message']);
