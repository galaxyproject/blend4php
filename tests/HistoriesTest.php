...
//     $this->assertTrue(is_array($history), $histories->getErrorMessage());
    ...
    // A test here to see if we can create a history from a local file on our
    // system, rather than copying a history or importing form a url we want
    // to grab a *.bed as a content or the tar ball to be uploaded to the
    // history

    //print_r($histories->archiveExport($history_list[0]['id']));

    $history = $histories->create('attemptfromlocalfile', 'file:///home/fish6/GalaxyPAPI/tests/files/test.bed.tar.gz', 'file', FALSE);
    print_r($histories->getErrorMessage());
    print_r($history);
